...
csv <- read.csv("20NC1_CC_ALT-Goldsboro_data.csv", check.names = FALSE) |>
  rename(Stage1 = `Growth Stage harvest 1`) |>
  rename(Stage2 = `Growth Stage Score - Harvest 2`)

xls <- read.xlsx("ForageQualityAnalysis Samples from NCxlsx.xlsx") |>
  rename(Sample = `Sample.Description.(unique.ID)`) |>
  filter(startsWith(Sample, "20NCCC"))
